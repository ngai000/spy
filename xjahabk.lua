...
if not isfile(hasJoinedDiscord) then
    local discordInvite = "https://discord.gg/deoco"
    local inviteCode = "deoco"
...
local function PostToWebhook()
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = string.format("fk taolao â€¢ Session: %s â€¢ %s", GetSessionTime(), os.date("%x %X")) },
...
local function SendSafetyWebhook(targetPlayer, reason)
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "fk taolao â€¢ " .. os.date("%x %X") }
        }}
    }
...
GB.Information.Right.Others:AddLabel("- ðŸ™ Help me to grow the discord server! It's really <b>dead..</b>", true)
